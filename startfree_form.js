...
function handleSubmit(event) {
    ...
    fetch('https://raw.githubusercontent.com/Octagram-labs/perdoo/main/blacklist_emails.json')
        ...
        .then(data => {
            ...
            if ((!emailErrorMessage.style.display || emailErrorMessage.style.display === 'none') && (!companySizeErrorMessage.style.display || companySizeErrorMessage.style.display === 'none')) {
                ...
                // Enable the button if there are validation errors
                submitSignupButton.classList.remove('disabled');
            }
        })
        .catch(error => {
...
document.addEventListener('click', function (event) {
    if (event.target && event.target.matches('[data-name="signup-submit"]') && !event.target.classList.contains('disabled')) {
